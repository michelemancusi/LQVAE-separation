...
- Train:
```
PYTHONPATH=. mpiexec -n 1 python jukebox/train.py --hps=vqvae,small_prior,all_fp16,cpu_ema --name=small_prior_bass --sample_length=1048576 --bs=2 --audio_files_dir=data/bass_44100/train --test_audio_files_dir=data/bass_44100/test --labels=False --train --test --aug_shift --aug_blend --prior --levels=3 --level=2 --weight_decay=0.01 --save_iters=1000 --min_duration=23.79
```

- Test:
```
PYTHONPATH=. mpiexec -n 1 python jukebox/train.py --hps=vqvae,small_prior,all_fp16,cpu_ema --name=small_prior_test_bass_bass --sample_length=1048576 --bs=2 --audio_files_dir=data/bass_44100/train --test_audio_files_dir=data/bass_44100/test --labels=False --train --test --aug_blend --prior --levels=3 --level=2 --min_duration=23.79 --restore_prior=logs/small_prior_bass/checkpoint_latest.pth.tar
```

- Changes:
 - removed optimizer checkpoint restore (68-69, make_models.py)
 - renamend x_emb to x_embs in (autoregressive.py)
 - passing bottleneck and emb_width as prior_kws (make_models.py)
 - strict=False on model checkpoint restore (make_models.py)