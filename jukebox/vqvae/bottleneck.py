...
class Bottleneck(nn.Module):
    ...
    def one_level_encode(self, xs):
        level_block = self.level_blocks[-1]
        zs = level_block.encode(xs)
        return zs

    def one_level_decode(self, zs, start_level=0, end_level=None):
        #print(zs.shape)
        if end_level is None:
            end_level = self.levels
        level_block = self.level_blocks[start_level:end_level][-1]
        xs_quantised = level_block.decode(zs)
        return xs_quantised
